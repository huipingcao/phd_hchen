...
def extract_clasp_cps(time_series, window_size, n_change_points=None, offset=.05):
    ...
    for idx in range(n_change_points):
        ...
        scores.append(-priority)  # -priority = the local max of profile score
        ...
        if len(left_range) > window_size:
            left_profile, _ = calc_clasp(time_series=time_series[left_range], window_size=window_size, )
        ...
        if len(right_range) > window_size:
            right_profile, _ = calc_clasp(time_series=time_series[right_range], window_size=window_size, )
